...
<div class="wrapper col2" style="height: 678px" >
 ...
 $tbl_name2 = "usuarios";
 $form_pass = $_POST["pass1"];
 $hash = password_hash($form_pass, PASSWORD_BCRYPT); 
 $conexion = new mysqli($host_db, $user_db, $pass_db, $db_name);
 if ($conexion->connect_error) {
 die("La conexion falló: " . $conexion->connect_error);
}
	$buscarUsuario = "SELECT * FROM $tbl_name WHERE nombre_administrador = '$_POST[username]' ";
	$result = $conexion->query($buscarUsuario);
	$count = mysqli_num_rows($result);
	
	$buscarUsuario2 = "SELECT * FROM $tbl_name2 WHERE nombre_usuario = '$_POST[username]' ";
	$result2 = $conexion->query($buscarUsuario2);
	$count2 = mysqli_num_rows($result2);
if ($count == 1 || $count2 == 1) {
	echo "<br /><h2 style='color:black;'>". "El Nombre de Usuario ya a sido tomado." . "</h2><br />";
	echo "<h2 style='color:black;'><a href='registrar_administrador.html'>Por favor escoga otro Nombre</a></h2>";
	}
	else{
		$query = "INSERT INTO administrador (nombre_administrador, password) VALUES ('$_POST[username]', '$hash')";
